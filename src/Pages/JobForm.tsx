...
function JobForm(jId: any) {
  ...
  const [jobSkillNames, setJobSkillNames] = useState<string[]>([]);
  const fetchJobJobSkills = async () => {
    try {
      ...
      const selectedSkills = response.data.result.map((skill:any) => skill.id);
      setValues(prevValues => ({
        ...prevValues,
        jobSkill: selectedSkills
      }));
      //setJobSkillNames(selectedSkills);
    } catch (error) {
  ...
  const handleJobSkillChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const { value, checked } = event.target;
    setValues(prevValues => ({
      ...prevValues,
      jobSkill: checked
        ? [...prevValues.jobSkill, value]
        : prevValues.jobSkill.filter(skill => skill !== value)
    }));
  };
  ...
  {jobSkillLoading ? (
    ...
    jobSkills.apiResponse.result.map((skill: Skill) => (
        <div key={skill.id}>
          <label>
            <input
              type="checkbox"
