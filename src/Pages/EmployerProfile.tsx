...
function EmployerProfile(props: Props) {
  ...
  const {
    ...
    error: jobError,
  } = useGetJobByEmployerIdQuery(id);

  useEffect(() => {
    if (!jobLoading && jobs && jobs.result) {
      console.log(jobs.result);
      setJobValue(jobs.result);
    }
  }, [jobLoading, jobs]);

  const { data, isLoading, isSuccess, error } = useGetEmployerByIdQuery(id);
  let content;
  if (isLoading) {
    content = <p>Loading ...</p>;
  } else if (error) {
        ...
        <div className="h-full bg-gray-200 p-8">
          ...
          <div className="flex flex-col items-center -mt-20">
            ...
            <div className="flex mt-2">
              <button
                className="mx-2"
                onClick={() => navigate(`/employerForm/${data.id}`)}
              >
                <FaEdit />
              </button>
